...
def plot_com_layer_stacking(atoms_com,num_of_layers,output_dir,height=5):
    '''
    功能
    ----------
    画com的堆叠情况
    ...
    def extract_layer_confs(atoms, zmin, zmax):
        ...
        sel = (zmin < z) & (z < zmax)
        return pos[sel]
    myfun.mkdir("%s/com_layer_pos"%output_dir)
    num_bins = 300
    layer_pos = list()
    atoms_com.wrap()
    ...
    fig.tight_layout()
    fig.savefig("%s/com_layer_pos/z_hist.png"%output_dir,dpi=320)
    plt.close("all")
    ...
    for j in range(len(peaks[1:1+num_of_layers])-2):
        ...
        axe.legend()
        fig.tight_layout()
        fig.savefig("%s/com_layer_pos/layer_pos_%d.png"%(output_dir,j),dpi=320)
        plt.close("all")

def plot_layer_stacking(atoms,num_of_layers,output_dir,height=5):
    '''
    功能
    ----------
    画com的堆叠情况
    ...
    z_centers = [0.5*(bins[j]+bins[j+1]) for j in peaks[1:1+num_of_layers]]
    for j in range(len(peaks[1:1+num_of_layers])):
        layer_confs = extract_layer_confs(atoms, z_centers[j]-cell_z/num_of_layers/2, z_centers[j]+cell_z/num_of_layers/2)
        layer_pos.append(layer_confs)
    for j in range(len(peaks[1:1+num_of_layers])):
        for k in range(j+1,len(peaks[1:1+num_of_layers])):
            fig, axe = plt.subplots(1, 1)
            axe.plot(layer_pos[j][:, 0], layer_pos[j][:, 1], linestyle="", marker="_", label="layer_%d" % (j+1))
            axe.plot(layer_pos[k][:, 0], layer_pos[k][:, 1], linestyle="", marker="|", label="layer_%d" % (k+1))
            axe.set_xlabel('x (A)')
            axe.set_ylabel('y (A)')
            axe.set_xlim(min(positions[:,0])-2,max(positions[:,1])+2)
            axe.set_ylim(min(positions[:,0])-2,max(positions[:,1])+2)
            ...
            axe.legend()
            fig.tight_layout()
            fig.savefig("%s/layer_pos/layer_pos_%d_%d.png"%(output_dir,j,k),dpi=320)
            plt.close("all")
